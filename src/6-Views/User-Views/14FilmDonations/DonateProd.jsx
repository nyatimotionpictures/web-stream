...
import { makeFilmDonation } from "../../../5-Store/TanstackStore/services/api";
...
const DonateProd = ({film, user}) => {
  ...
  const validationSchema = yup.object().shape({
    ...
    paymentNumber: yup.string().required("required"),
     option: yup.string().required("select payment method to continue"),
  ...
  useEffect(() => { 
    if(film !== null){
      // setSelectedAmount(() => film?.donationTargetAmount)
    }
  },  [film])

  let initialValues = film !== null  ? {
    filmId: film?.id,
    userId: user?.id,
    amount: null,
    option: "mtnmomo",
  ...
  return (
    <div className="min-h-[60vh] h-full lg:min-h-screen flex flex-col bg-[#17141B] items-center justify-center px-[30px] py-16 sm:px-16 md:py-16 lg:py-16 w-screen overflow-hidden relative">
      <Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={async (values, helpers) => {
                ...
                <div className="flex flex-col w-full gap-[10px]">
                  ...
                  <SingleWrapper>
                    <FormContainer className="w-full">
                      ...
                      <ErrorMessage
                        errors={errors && errors?.paymentNumber ? true : false}
                        message={errors?.paymentNumber}
                      />
                    </FormContainer>
                  </SingleWrapper>
                  {/** First Name */}
                  ...
                  <SingleWrapper>
                    <FormContainer className="w-full">
                      <label htmlFor="film" className="text-[#bdb8b8] font-[Roboto-Medium] text-sm">
                        Film
                      </label>
