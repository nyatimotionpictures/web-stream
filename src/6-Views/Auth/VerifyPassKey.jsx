...
import { postSendOtp, verifyOtp } from "../../5-Store/TanstackStore/services/api";
...
const VerifyPassKey = () => {
  ...
  React.useEffect(() => {
    otp0.current?.focus();
    sendMutation.mutate({contact: location?.state?.contact, isEmail: location?.state?.isEmail});
  }, []);
  ...
  const verifyMutation = useMutation({
    ...
    onSuccess: (data) => {
      setSnackbarMessage({message: data.message, severity: "success"});
      navigate("/success", { replace: true, state:{contact:location.state.contact, isEmail: location.state.isEmail  } });
    },
    onError: (error) => {
      setSnackbarMessage({message: error?.message, severity: "error"});
    },
  ...
  const handleSubmitOtp = () => {
    ...
    verifyMutation.mutate({contact, isEmail, otp: otpnum?.join("")});
                    ...
                    <p className="text-[#F2F2F2] text-[14px] text-center select-none font-[Inter-Regular]">
                      Resend code:{" "}
                      {
                        sendMutation.isPending ? (
                          ...
                          <span onClick={() => handleResendOtp()} className="text-[#ED3F62] underline underline-offset-2 cursor-pointer">
                          send another code
                        </span>
                        )
                      }
                ...
                {verifyMutation.isPending? (
                  ...
                  onClick={otpnum?.includes("") ? null : handleSubmitOtp}
                    ...
                    className={`block w-full rounded-full border border-[#EE5170] bg-primary-400 px-12 py-3 text-sm font-medium text-whites-40 text-[14.35px] hover:bg-primary-500 hover:text-white focus:outline-none focus:ring active:text-opacity-75 sm:w-auto lg:px-16 font-[Inter-SemiBold] ${
                        otpnum?.includes("") ? " text-opacity-50 border-secondary-300 bg-secondary-300 !bg-opacity-50 !cursor-not-allowed hover:bg-secondary-300" : ""
                      }`}
                  >
                      Continue
                  </Button>
