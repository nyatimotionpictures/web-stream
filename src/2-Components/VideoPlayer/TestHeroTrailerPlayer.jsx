...
const TestHeroTrailerPlayer = ({ 
  ...
  const [abortController, setAbortController] = useState(null);
    ...
    useEffect(() => {
      ...
      return () => {
        controller.abort();
        console.log('🎬 HeroTrailerPlayer: Abort controller cleaned up');
      };
    }, []);
  ...
  const pauseOtherPlayers = () => {
    for (const [id, player] of globalPlayerState.activePlayers) {
      if (id !== playerId) {
        player.pause();
      }
    }
    globalPlayerState.currentPlayingId = playerId;
  }
  ...
  useEffect(() => {
    ...
    const setupVideo = async () => {
      try {
        ...
        const handlePlay = () => {
          ...
          //pauseOtherPlayers(); // Pause other players when this one plays
          if (onPlay) onPlay();
        };
    ...
    const cleanupEventListeners = setupVideo();
  ...
  useEffect(() => {
    ...
    video.preload = isVisible ? 'auto' : 'none';
  ...
  }, [muted, loop, showControls, isVisible]);
...
export default TestHeroTrailerPlayer;